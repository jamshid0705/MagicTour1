...
const login = catchError(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(
      new appError('Siz email bn password kiritishingiz kerak!!!', 404)
    );
  }
...
const protect = catchError(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new appError('Bunday user mavjud emas. Royhatdan otib kirin', 401)
    );
  }
  ...
  if (user.passwordChangedDate) {
    ...
    if (user.passwordChangedDate.getTime() / 1000 > tokencha.exp) {
      return next(
        new appError(
          'Sizning tokeningiz yaroqsiz! iltimos qaytadan bazaga kiring !'
        )
      );
    }
  }
...
const role = (roles) => {
  return catchError(async (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new appError('Siz bu userni ochirish huquqiga ega emassiz !', 404)
      );
    }
    next();
  });
...
const forgotpassword = catchError(async (req, res, next) => {
  ...
  // 4 emailga jo'natish
  const resentLink = `${req.protocol}://${req.get(
    'host'
  )}/api/v1/users/resentpassword/${token}`;
