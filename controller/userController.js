...
const updateMePassword=catchError(async(req,res,next)=>{
  ...
  if(req.body.oldpassword===req.body.newpassword){
    ...
    return next(
      new appError('Siz bir xil parol kiritdingiz ! ',401)
    );
  }
  ...
  if(!req.body.oldpassword){
    return next(
      new appError('Siz eski parolni kirtishingiz shart ! ',401)
    );
  }
  ...
  if (!tekshir) {
    return next(
      new appError('Notogri parol kiritidng ! ',401)
    );
  }
  ...
  if(req.body.newpassword!=req.body.newpasswordConfirm){
    return next(
      new appError('Siz bir xil parol kiriting ! ',401)
    );
  }
